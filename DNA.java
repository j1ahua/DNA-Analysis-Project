...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        for (int i = 0; i < database.length; i++){
            ...
            database[i] = new Profile(name, S1_STRs, S2_STRs, sequence1, sequence2);
        }
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        STRsOfInterest = new String[numOfSTR];
        StdIn.readLine();
	...
	/* WRITE YOUR CODE HERE */
        ...
        STR[] S1_STRs = null;
        STR[] S2_STRs = null;
        Profile UnknownProfile = new Profile("Unknown", S1_STRs, S2_STRs, seq1, seq2);
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        STR STRinSequence = new STR(STR, maxLength);
        return STRinSequence; // update the return value1
    }
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        ...
        int numOfSTR = allSTRs.length;
        STR[] S1_STRs = new STR[numOfSTR];
        STR[] S2_STRs = new STR[numOfSTR];
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        /* WRITE YOUR CODE HERE */
        ArrayList<Profile> potentialProfiles = new ArrayList<Profile>();
        for(int i = 0; i < database.length; i++){
            if(identicalSTRs(unknownProfileS1_STRs, database[i].getS1_STRs())){
                potentialProfiles.add(database[i]);
            }
        }
        

	return potentialProfiles; // update the return value
    }
    ...
    public ArrayList<Profile> findPossibleParents ( STR[] S1_STRs, STR[] S2_STRs ) {
        ...
        ArrayList<Profile> parentList = new ArrayList<Profile>();
         ...
         for ( int p1 = 0; p1 < possibleParent1.size(); p1++ ) {
            ...
            for ( int p2 = 0; p2 < possibleParent2.size(); p2++ ) {
                ...
                if ( !possibleParent1.get(p1).equals(possibleParent2.get(p2)) ) {
                    ...
                    if ( punnetSquare(possibleParent2.get(p2).getS2_STRs(), S2_STRs, possibleParent1.get(p1).getS1_STRs(), S1_STRs)) {
